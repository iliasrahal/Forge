...
export default function MarketplaceCard({ posting, applicationCount }: { posting: MarketplacePublicPosting; applicationCount?: number }) {
  ...
  return <article className="forge-surface flex h-full flex-col rounded-3xl border p-5">
    <div className="flex items-start justify-between gap-3"><span className="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-700 dark:bg-blue-950 dark:text-blue-300">{posting.trade}</span><span className="text-xs font-semibold text-[var(--forge-text-muted)]">{formatMarketplaceStatus(posting.status)}</span></div>
    ...
    <p className="mt-3 text-xs font-semibold text-[var(--forge-text-muted)]">{posting.publisher}{applicationCount === undefined ? "" : ` · ${applicationCount} demande${applicationCount > 1 ? "s" : ""}`}</p>
