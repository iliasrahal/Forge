...
export default function MarketplaceCard({ posting, applicationCount, uniqueViewCount }: { posting: MarketplacePublicPosting; applicationCount?: number; uniqueViewCount?: number }) {
  ...
  return <article className="forge-surface flex h-full flex-col rounded-3xl border p-5">
    ...
    <div className="mt-3 space-y-1.5">{posting.requirements.slice(0,3).map((requirement) => <div key={requirement.id} className="flex items-center justify-between gap-2 text-xs text-[var(--forge-text-secondary)]"><span className="font-semibold">{requirement.trade} ×{requirement.requiredCount}</span><span>{date(requirement.startDate)} → {date(requirement.endDate)}</span></div>)}{posting.requirements.length > 3 ? <p className="text-xs font-semibold text-blue-600">+ {posting.requirements.length - 3} autre{posting.requirements.length > 4 ? "s" : ""}</p> : null}</div>
