...
export default function InterventionProfitability({ interventionId, canWrite, hourlyCostCents, metrics, expenses, workTimes }: { interventionId: string; canWrite: boolean; hourlyCostCents: number | null; metrics: Metrics; expenses: Expense[]; workTimes: WorkTime[] }) {
  ...
  return <section className="mx-auto mt-8 max-w-2xl rounded-3xl border border-blue-200/70 bg-white/45 p-5 dark:border-blue-800/60 dark:bg-slate-900/35">
    ...
    {form === "expense" && <form onSubmit={expenseSubmit} className="mt-4 grid gap-3 sm:grid-cols-2"><input required name="amount" inputMode="decimal" defaultValue={editingExpense ? editingExpense.amountCents / 100 : ""} placeholder="Montant (€)" className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70"/><select name="category" defaultValue={editingExpense?.category ?? "MATERIALS"} className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70">{Object.entries(labels).map(([value,label])=><option key={value} value={value}>{label}</option>)}</select><input required name="date" type="date" defaultValue={editingExpense?.date ?? today} className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70"/><input name="supplier" defaultValue={editingExpense?.supplier ?? ""} placeholder="Fournisseur" className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70"/><textarea name="description" defaultValue={editingExpense?.description ?? ""} placeholder="Description facultative" className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70 sm:col-span-2"/><button disabled={pending} className="rounded-full bg-blue-600 px-4 py-2 font-semibold text-white sm:col-span-2">Enregistrer</button></form>}
    {form === "time" && <form onSubmit={timeSubmit} className="mt-4 grid gap-3 sm:grid-cols-3"><input required name="date" type="date" defaultValue={editingTime?.date ?? today} className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70"/><input required name="hours" type="number" min="0" max="24" defaultValue={editingTime?.durationMinutes ? Math.floor(editingTime.durationMinutes / 60) : ""} placeholder="Heures" className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70"/><input required name="minutes" type="number" min="0" max="59" defaultValue={editingTime?.durationMinutes ? editingTime.durationMinutes % 60 : 0} placeholder="Minutes" className="rounded-xl border bg-white/70 px-3 py-2 dark:bg-slate-900/70"/><button disabled={pending} className="rounded-full bg-blue-600 px-4 py-2 font-semibold text-white sm:col-span-3">Enregistrer</button></form>}
