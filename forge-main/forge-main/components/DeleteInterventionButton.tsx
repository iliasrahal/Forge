...
export default function DeleteInterventionButton({ interventionId }: { interventionId: string }) {
      ...
      {open && (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-slate-950/45 p-4 backdrop-blur-sm">
          <section role="dialog" aria-modal="true" aria-labelledby="delete-intervention-title" className="forge-surface w-full max-w-sm rounded-[2rem] border p-6 text-center">
            <h2 id="delete-intervention-title" className="text-xl font-bold text-[var(--forge-text-primary)]">Supprimer ce chantier ?</h2>
            <p className="mt-2 text-sm leading-6 text-[var(--forge-text-secondary)]">Cette action supprimera le chantier et son planning associé.</p>
            ...
            <div className="mt-6 grid grid-cols-2 gap-3">
              ...
              <button type="button" disabled={pending} onClick={() => void remove()} className="min-h-12 rounded-2xl bg-red-600 px-4 font-semibold text-white transition hover:bg-red-700 disabled:opacity-50">{pending ? "Suppression…" : "Supprimer"}</button>
            </div>
          </section>
        </div>
      )}
