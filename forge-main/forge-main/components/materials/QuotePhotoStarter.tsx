...
export default function QuotePhotoStarter({ initialSource, onClose, onPreparedLines }: QuotePhotoStarterProps = {}) {
  ...
  const [mode, setMode] = useState<"idle" | "photo">(initialSource ? "photo" : "idle");
  ...
  useEffect(() => {
    if (!initialSource || sourceOpened.current) return;
    sourceOpened.current = true;
    const input = initialSource === "camera" ? cameraRef.current : galleryRef.current;
    input?.click();
  }, [initialSource]);
  ...
  async function addPhotos(files: FileList | null, analyzeAfterAdd = false) {
    ...
    try {
      ...
      if (compressed.length) setMode("photo");
  ...
  function chooseMaterial(material: QuoteMaterialSnapshotSource) {
    setShowPicker(false);
    setPrepareWithoutMaterial(false);
    setSelectedMaterial(material);
  }

  function finishPreparation(lines: EditableQuoteLine[], title: string) {
    if (onPreparedLines) {
  ...
  if (mode === "idle") {
    return (
      <section className="mb-4 grid grid-cols-1 gap-2 sm:flex" aria-label="Analyse photo du matériel">
        <input ref={cameraRef} hidden type="file" accept="image/*" capture="environment" onChange={(event) => { void addPhotos(event.target.files); event.currentTarget.value = ""; }} />
        <input ref={galleryRef} hidden type="file" accept="image/*" multiple onChange={(event) => { void addPhotos(event.target.files); event.currentTarget.value = ""; }} />
        <button type="button" onClick={() => startWith("camera")} className="forge-surface flex min-h-14 w-full items-center justify-center gap-3 rounded-2xl border px-4 py-3 text-center font-semibold text-[var(--forge-text-primary)] hover:border-blue-400 sm:w-auto">Prendre une photo</button>
        <button type="button" onClick={() => startWith("gallery")} className="forge-surface flex min-h-14 w-full items-center justify-center gap-3 rounded-2xl border px-4 py-3 text-center font-semibold text-[var(--forge-text-primary)] hover:border-blue-400 sm:w-auto">Ajouter depuis la galerie</button>
      </section>
    );
  }
  ...
  return (
    <section className="forge-surface mb-6 rounded-3xl border p-4 sm:p-6">
      ...
      {!result ? <div className="mt-4 grid grid-cols-1 gap-2 min-[380px]:grid-cols-2"><button type="button" onClick={() => cameraRef.current?.click()} className="min-h-12 rounded-xl bg-blue-600 px-3 text-center font-semibold text-white">Prendre une photo</button><button type="button" onClick={() => galleryRef.current?.click()} className="min-h-12 rounded-xl border border-[var(--forge-border-strong)] px-3 text-center font-semibold text-[var(--forge-text-primary)]">Ajouter depuis la galerie</button></div> : null}
...
<button type="button" disabled={!photos.length || loading} onClick={() => void analyze()} className="mt-3 min-h-12 w-full rounded-xl bg-gradient-to-r from-blue-600 to-pink-500 px-4 font-semibold text-white disabled:opacity-50">{loading ? "Analyse en cours…" : "Analyser le matériel"}</button>
