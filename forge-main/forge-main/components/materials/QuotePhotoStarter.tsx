...
export default function QuotePhotoStarter() {
  ...
  return (
    <section className="forge-surface mb-6 rounded-3xl border p-4 sm:p-6">
      ...
      {result ? <div className="mt-5 space-y-4"><div className="rounded-2xl bg-[var(--forge-surface-secondary)] p-4"><p className="font-bold text-[var(--forge-text-primary)]">Forge a identifié : {result.identification.equipmentType || "matériel à confirmer"}</p><p className="mt-1 text-sm text-[var(--forge-text-muted)]">Confiance {result.identification.confidence === "high" ? "élevée" : result.identification.confidence === "medium" ? "moyenne" : "faible"}. {[result.identification.brand, result.identification.reference].filter(Boolean).join(" · ")}</p></div>
        {result.identification.visibleCharacteristics.length ? <div><h3 className="font-semibold text-[var(--forge-text-primary)]">Caractéristiques visibles</h3><p className="mt-1 text-sm text-[var(--forge-text-secondary)]">{result.identification.visibleCharacteristics.map((item) => `${item.name} : ${item.value}`).join(" · ")}</p></div> : null}
        {result.identification.uncertainCharacteristics.length ? <div className="rounded-xl bg-amber-500/10 p-3 text-sm text-amber-800 dark:text-amber-200"><strong>À confirmer :</strong> {result.identification.uncertainCharacteristics.map((item) => `${item.name} (${item.possibleValue})`).join(" · ")}</div> : null}
        {result.identification.questions.length ? <div><h3 className="font-semibold text-[var(--forge-text-primary)]">Forge a besoin d’une précision</h3><ul className="mt-1 list-disc pl-5 text-sm text-[var(--forge-text-secondary)]">{result.identification.questions.map((question) => <li key={question}>{question}</li>)}</ul><p className="mt-2 text-sm text-[var(--forge-text-muted)]">Ajoute une photo ciblée ou précise la réponse dans le contexte, puis relance l’analyse.</p></div> : null}
        <div><h3 className="font-semibold text-[var(--forge-text-primary)]">Références compatibles du catalogue</h3>{result.matches.length ? <div className="mt-2 space-y-2">{result.matches.map(({ material, reasons }) => <button key={material.id} type="button" onClick={() => chooseMaterial(material)} className="flex w-full items-center justify-between gap-3 rounded-2xl border border-[var(--forge-border)] bg-[var(--forge-surface-secondary)] p-4 text-left hover:border-blue-400"><span><span className="block font-semibold text-[var(--forge-text-primary)]">{material.name}</span><span className="block text-sm text-[var(--forge-text-muted)]">{[material.brand, material.reference].filter(Boolean).join(" · ")} · {reasons.slice(0, 2).join(", ")}</span></span><span className="shrink-0 font-bold text-blue-600">Choisir</span></button>)}</div> : <p className="mt-2 rounded-xl bg-[var(--forge-surface-secondary)] p-3 text-sm text-[var(--forge-text-muted)]">Aucune référence du catalogue ne correspond assez précisément.</p>}<button type="button" onClick={() => setShowPicker(true)} className="mt-3 min-h-11 w-full rounded-xl border border-[var(--forge-border-strong)] font-semibold text-[var(--forge-text-primary)]">Rechercher manuellement</button></div>
      </div> : null}
