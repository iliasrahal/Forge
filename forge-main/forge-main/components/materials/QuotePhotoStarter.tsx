...
export default function QuotePhotoStarter({ initialSource, onClose, onSelectMaterial }: QuotePhotoStarterProps = {}) {
  ...
  if (mode === "idle") {
    return (
      <section className="mb-4 grid grid-cols-1 gap-2 sm:flex" aria-label="Analyse photo du matériel">
        ...
        <button type="button" onClick={() => startWith("camera")} className="forge-surface flex min-h-14 w-full items-center justify-center gap-3 rounded-2xl border px-4 py-3 text-center font-semibold text-[var(--forge-text-primary)] hover:border-blue-400 sm:w-auto sm:justify-start"><Camera className="text-pink-500" />Prendre une photo</button>
        <button type="button" onClick={() => startWith("gallery")} className="forge-surface flex min-h-14 w-full items-center justify-center gap-3 rounded-2xl border px-4 py-3 text-center font-semibold text-[var(--forge-text-primary)] hover:border-blue-400 sm:w-auto sm:justify-start"><ImagePlus className="text-blue-500" />Ajouter depuis la galerie</button>
      </section>
    );
  }
  ...
  return (
    <section className="forge-surface mb-6 rounded-3xl border p-4 sm:p-6">
      ...
      {!result ? <div className="mt-4 grid grid-cols-1 gap-2 min-[380px]:grid-cols-2"><button type="button" onClick={() => cameraRef.current?.click()} className="min-h-12 rounded-xl bg-blue-600 px-3 font-semibold text-white"><Camera className="mr-2 inline" size={18} />Prendre une photo</button><button type="button" onClick={() => galleryRef.current?.click()} className="min-h-12 rounded-xl border border-[var(--forge-border-strong)] px-3 font-semibold text-[var(--forge-text-primary)]"><ImagePlus className="mr-2 inline" size={18} />Ajouter depuis la galerie</button></div> : null}
