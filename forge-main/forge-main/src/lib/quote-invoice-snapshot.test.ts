...
test("copie toutes les données tarifaires sans partager les lignes", () => {
  const quote = {
    ...
    discountBp: 500,
    totalCostCents: 9000,
    lines: [{ id: "quote-line-1", quoteId: "quote-1", createdAt: new Date(), category: "Main d'œuvre", label: "Main d'œuvre", quantityMilli: 2000, unit: "h", unitPriceCents: 5000, costCents: 2000, discountBp: 0, amountCents: 10000, vatRateBp: 1000, details: [{ label: "Pose", description: null, quantityMilli: 3000, unit: "u", unitPriceCents: 2000, amountCents: 6000, position: 0 }] }],
  };
  const snapshot = buildInvoiceSnapshotFromQuote(quote);
  assert.deepEqual(snapshot.lines, [{ category: "Main d'œuvre", label: "Main d'œuvre", quantityMilli: 2000, unit: "h", unitPriceCents: 5000, costCents: 2000, discountBp: 0, amountCents: 10000, vatRateBp: 1000, details: { create: [{ label: "Pose", description: null, quantityMilli: 3000, unit: "u", unitPriceCents: 2000, amountCents: 6000, position: 0 }] } }]);
