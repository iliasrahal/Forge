...
test("copie toutes les données tarifaires sans partager les lignes", () => {
  const quote = {
    ...
    discountBp: 500,
    totalCostCents: 9000,
    lines: [{ category: "Main d'œuvre", label: "Main d'œuvre", quantityMilli: 2000, unit: "h", unitPriceCents: 5000, costCents: 2000, discountBp: 0, amountCents: 10000, vatRateBp: 1000 }],
  };
  const snapshot = buildInvoiceSnapshotFromQuote(quote);
  assert.deepEqual(snapshot.lines, quote.lines);
